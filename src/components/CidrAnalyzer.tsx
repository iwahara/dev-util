import { HStack, VStack, Input, InputGroup, InputRightAddon, NumberInput, NumberInputField, Button, useToast } from "@chakra-ui/react";
...
function CidrAnalyzer(){
    ...
    function commandCidrAnalyzer(){
        ...
        invoke<CidrAnalyzerResponse>('command_cidr_analyzer', req).then(res => {
            console.log(res);
            setResponse(res);
        }).catch(message => {
    ...
    return (
        <div>
            <VStack align="left">
                <InputGroup>
                    <NumberInput min={0} max={255} onChange={(value) => setFirstOctet(Number(value))}>
                        <NumberInputField />
                    </NumberInput>
                    <InputRightAddon children='.' />
                    <NumberInput min={0} max={255} onChange={(value) => setSecondOctet(Number(value))}>
                        <NumberInputField />
                    </NumberInput>
                    <InputRightAddon children='.' />
                    <NumberInput min={0} max={255} onChange={(value) => setThirdOctet(Number(value))}>
                        <NumberInputField />
                    </NumberInput>
                    <InputRightAddon children='.' />
                    <NumberInput min={0} max={255} onChange={(value) => setFourthOctet(Number(value))}>
                        <NumberInputField />
                    </NumberInput>
                    <InputRightAddon children='/' />
                    <NumberInput min={0} max={32} onChange={(value) => setPrefixLen(Number(value))}>
                        <NumberInputField />
                    </NumberInput>
                </InputGroup>
                <Button onClick={commandCidrAnalyzer} colorScheme='blue' disabled={buttonDisabled}>解析する</Button>
                {response &&
                    <TableContainer>
                        <Table variant='simple' colorScheme="blue">
                            ...
                            <Tr>
                                <Td>IPアドレスの範囲</Td>
                                <Td>{response?.ip_address_range_start} - {response?.ip_address_range_end}</Td>
                            </Tr>
                            <Tr>
                                <Td>IPアドレス数</Td>
